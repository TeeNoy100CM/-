...
def get_status_changes():
    ...
    # ✅ ฟังก์ชันสร้างข้อความพร้อมสี HTML
    def make_colored_arrow(row):
        p_status = row['Prev_Status']
        c_status = row['Status']

        # กำหนดสีตามสถานะ
        p_color = "red" if p_status == "OFFLINE" else "green"
        c_color = "red" if c_status == "OFFLINE" else "green"

        # สร้าง HTML String (ใช้ <span> เพื่อใส่สี)
        return f'<span style="color:{p_color};">{p_status}</span> ➡️ <span style="color:{c_color};font-weight:bold;">{c_status}</span>'

    changes_df['Event'] = changes_df.apply(make_colored_arrow, axis=1)
    changes_df = changes_df.sort_values('Timestamp', ascending=False)

    return changes_df[['Timestamp', 'Label', 'Event', 'IP']]
def get_status_changes():
    """ดึงข้อมูลการเปลี่ยนสถานะและแสดงเป็นลูกศรทิศทาง (เช่น OFF -> ON)"""
    if not os.path.isfile(HISTORY_FILE):
        return None
    ...
    # 3. กรองเฉพาะแถวที่สถานะเปลี่ยน (และไม่ใช่บรรทัดแรกที่ไม่มีตัวเทียบ)
    changes_df = df_history[
        (df_history['Prev_Status'].notna()) &
        (df_history['Status'] != df_history['Prev_Status'])
    ].copy()

    # 4. สร้างคอลัมน์แสดงการเปลี่ยนแปลงเป็นลูกศร
    def make_arrow(row):
        return f"{row['Prev_Status']} ➡️ {row['Status']}"

    changes_df['Event'] = changes_df.apply(make_arrow, axis=1)

    # 5. เรียงกลับเอาล่าสุดขึ้นก่อน เพื่อให้ข้อมูลค้างไว้ในตารางประวัติ
    changes_df = changes_df.sort_values('Timestamp', ascending=False)

    return changes_df[['Timestamp', 'Label', 'Event', 'Status', 'IP']]

    # คำนวณ Uptime % รายเครื่อง
    stats = []
    for label in history_df["Label"].unique():
        device_data = history_df[history_df["Label"] == label]
        total_scans = len(device_data)
        online_scans = len(device_data[device_data["Status"] == "ONLINE"])
        uptime_pct = (online_scans / total_scans) * 100
        stats.append({
            "Label": label,
            "Total Checks": total_scans,
            "Online Count": online_scans,
...
targets_combined = [
    ...
    { "ip": "1.0.203.225", "port": 80, "lat": 8.17664, "lon": 98.78512, "label": "โรงเรียนบ้านคลองทรายประชาอุทิศ จ.กระบี่" },
    { "ip": "101.109.255.222", "port": 80, "lat": 9.3056, "lon": 98.404, "label": "โรงเรียนบ้านสวนใหม่ จ.พังงา" },
    { "ip": "171.102.131.16", "port": 18000, "lat": 18.9848, "lon": 98.941, "label": "วัดป่าน้ำริน เเม่ริม จ.เชียงใหม่" },
    { "ip": "171.102.131.15", "port": 18000, "lat": 18.8954, "lon": 99.1435, "label": "วัดป่าจิตตภาวนาราม ดอยสะเก็ด จ.เชียงใหม่" },
    { "ip": "171.102.131.22", "port": 18000, "lat": 18.8947, "lon": 98.9659, "label": "วัดสมเด็จอาสภวัน เเม่เเตง จ.เชียงใหม่" },
    { "ip": "171.102.131.20", "port": 18000, "lat": 18.8392, "lon": 99.2056, "label": "โรงเรียนบ้านตลาดขี้เหล็ก ดอยสะเก็ด จ.เชียงใหม่" },
    { "ip": "171.102.131.21", "port": 18000, "lat": 18.6453, "lon": 98.8856, "label": "องค์การบริหารส่วนตำบลสันกลาง สันป่าตอง จ.เชียงใหม่" },
    { "ip": "171.102.131.19", "port": 18000, "lat": 18.8028, "lon": 98.9528, "label": "ภาควิชาธรณีวิทยา มหาวิทยาลัยเชียงใหม่ อ.เมือง จ.เชียงใหม่" },
    { "ip": "171.102.131.18", "port": 18000, "lat": 18.9712, "lon": 98.9103, "label": "วัดป่าเทพขันติราม (พระเมืองแก้ว) เเม่ริม จ.เชียงใหม่" },
    { "ip": "171.102.131.24", "port": 18000, "lat": 18.8982, "lon": 98.905, "label": "วัดป่าดอยวิเวก เเม่ริม จ.เชียงใหม่" },
    { "ip": "171.102.131.11", "port": 18000, "lat": 18.9033, "lon": 99.1949, "label": "วัดป่าศาลาปางสัก (วัดหลวงตา) ดอยสะเก็ด จ.เชียงใหม่" },
    { "ip": "171.102.131.93", "port": 18000, "lat": 19.0917, "lon": 98.9992, "label": "วัดเทพนิมิตรนันทาราม เเม่เเตง จ.เชียงใหม่" },
    { "ip": "171.102.131.10", "port": 18000, "lat": 18.7941, "lon": 99.1132, "label": "วัดป่าแพ่ง ดอยสะเก็ด จ.เชียงใหม่" },
    { "ip": "171.102.131.98", "port": 18000, "lat": 19.1451, "lon": 98.8563, "label": "วัดต้นลุงศรีดงเย็น (วัดต้นลุง) เเม่เเตง จ.เชียงใหม่" },
    { "ip": "171.102.131.26", "port": 18000, "lat": 9.45095, "lon": 100.034, "label": "เกาะสมุย จ.สุราษฎร์ธานี" },
    { "ip": "1.10.141.182", "port": 800, "lat": 18.1351, "lon": 100.2504, "label": "โรงเรียนบ้านแม่แคม แพร่ อุตรดิตถ์" },
    { "ip": "101.109.112.2", "port": 800, "lat": 17.3080, "lon": 100.6391, "label": "โรงเรียนบ้านนาเปอะ พิษณุโลก อุตรดิตถ์" }
...
if "scan_df" in st.session_state:
    ...
    tab1, tab2 = st.tabs(["🌐 แผนที่ปัจจุบัน", "📊 สถิติความเสถียร (Uptime)"])
    ...
    with tab2:
        ...
        if change_log is not None and not change_log.empty:
            display_df = change_log.copy()

            # ✅ ฟังก์ชันจัดการสี: แยกเขียว-แดงตามสถานะจริง
            def apply_color(row):
                # ตรวจสอบสถานะคำสุดท้ายใน Event (ตัวอย่าง: "OFFLINE ➡️ ONLINE")
                current_status = row['Event'].split('➡️')[-1].strip()

                # กำหนดสีตามสถานะ
                if current_status == 'ONLINE':
                    color = '#2ecc71' # สีเขียวสว่าง
                else:
                    color = '#e74c3c' # สีแดงสว่าง

                # คืนค่าสไตล์: ตัวอักษรตามสีสถานะ, พื้นหลังดำใสเพื่อให้มองง่าย
                return [f'color: {color}; font-weight: bold; background-color: rgba(0,0,0,0.3);' if i == 2 else '' for i in range(len(row))]

            # แสดงตาราง
            st.dataframe(
                display_df[['Timestamp', 'Label', 'Event', 'IP']].style.apply(apply_color, axis=1),
                use_container_width=True,
                height=600
            )
        else:
            st.info("⌛ ระบบกำลังรอการสลับสถานะครั้งต่อไป...")
